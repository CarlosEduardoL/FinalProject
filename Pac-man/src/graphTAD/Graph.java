			...
			for(K key2 : keys) {
				for(K key3 : keys) {
					...
					if (num1 > num2) {
						...
						List<V> l;
						l = new ArrayList<>(pathArray.get(key2).get(key1));
						if (l.size() > 0) {
							l.remove(l.size()-1);
						}
